...
const MessageForm: FC<MessageForm> = ({
  label,
  placeholder,
  message,
  sending,
  ...
  return (
    <form
      className={`w-full items-center flex outline-0 pr-1 border-[1px] ${
    ...
    >
      ...
      >
        {sending ? (
          ...
          <div
            className={`${
              (message.replace(/\s{1,}/g, "").length === 0 &&
                "bg-[rgb(100,116,139)] cursor-not-allowed") ||
              "hover:bg-[#CBA1A4] bg-[#ab8386] hover:cursor-pointer"
            } rounded-[50px] px-3 text-white h-full flex items-center`}
          >
            {label}
          </div>
        )}
